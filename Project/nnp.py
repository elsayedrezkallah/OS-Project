...
def npp(arrival_time: List[int], burst_time: List[int], priorities: List[int], process_names: List[str]) -> Dict[str, Any]:
    processes_info = [
        {
    ...
    for i in range(len(processes_info)):
        if i == 0:
            ...
            # Sort ready queue by priority and then by arrival time
